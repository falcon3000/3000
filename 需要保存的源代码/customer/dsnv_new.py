...
#import gpio
password = '111111'
# os.system("echo %s | sudo cat /sys/kernel/debug/gpio| grep '266'" % password)
os.system("echo %s | sudo -S chmod 777 /sys/class/gpio/export" % password)
os.system("echo %s | sudo -S chmod 777 /sys/class/gpio/unexport" % password)
...
def set_height_266(gpio):
    global IN_STATUS_266
    if IN_STATUS_266:
        gpio.set(tst_gpio_pin_266, 1)
        print("D1 TEST High.")
        time.sleep(5)
        gpio.set(tst_gpio_pin_266, 0)
        IN_STATUS_266 = False

def set_height_267(gpio):
    global IN_STATUS_267
    if IN_STATUS_267:
        gpio.set(tst_gpio_pin_267, 1)
        print("D0 TEST High.")
        time.sleep(5)
        gpio.set(tst_gpio_pin_267, 0)
        IN_STATUS_267 = False
                ...
                try:
                    ...
                    if user_meta.base_meta.meta_type == pyds.nvds_get_user_meta_type("NVIDIA.DSANALYTICSOBJ.USER_META"):
                        ...
                        if user_meta_data.roiStatus:
                            if frame_meta.pad_index == int(0):
                                # print("Object {0} roi status: {1}".format(obj_meta.object_id, user_meta_data.roiStatus))
                                #os.system("echo 'deepstream_run' >> test.log")
                                if not IN_STATUS_266:
                                    os.system("echo 'ori deepstream_run' >> test.log")
                                    IN_STATUS_266 = True
                                    print("初始化报警设置，启动D1:",IN_STATUS_266)
                                    status_266 = threading.Thread(target=set_height_266, args=(gpio, ))
                                    status_266.setDaemon(True)
                                    status_266.start()
                            else:
                                # print("Object {0} roi status: {1}".format(obj_meta.object_id, user_meta_data.roiStatus))
                                #os.system("echo 'deepstream_run' >> test.log")
                                if not IN_STATUS_267:
                                    os.system("echo 'ori deepstream_run' >> test.log")
                                    IN_STATUS_267 = True
                                    print("初始化报警设置，启动D0:",IN_STATUS_267)
                                    status_267 = threading.Thread(target=set_height_267, args=(gpio, ))
                                    status_267.setDaemon(True)
                                    status_267.start()
                except StopIteration:
                    break
...
def main(args, gpio):
    ...
    #pgie.set_property("filter-out-class-ids", 61;63)
    #pgie.set_property("filter-out-class-ids", 61;63)
    #filter-out-class-ids=61;63
    ...
    pgie.set_property('config-file-path', "dsnvanalytics_pgie_yolo_config.txt")
    ...
    #sink.set_property("overlay-x", 300)
    #sink.set_property("overlay-y", 100)
    #sink.set_property("overlay-w", 640)
    #sink.set_property("overlay-h", 480)
    sink.set_property("overlay", 1)
    sink.set_property("sync", 0)
    sink.set_property("qos", 0)
    sink.set_property("display-id", 1)
    ...
    if not nvanalytics_src_pad:
        ...
        nvanalytics_src_pad.add_probe(Gst.PadProbeType.BUFFER, nvanalytics_src_pad_buffer_probe, gpio)
...
if __name__ == '__main__':
    tst_gpio_pin_266 = 266
    #266 对应 T506S 扩展的 D1
    gpio.setup(tst_gpio_pin_266, gpio.OUT)
    gpio.set(tst_gpio_pin_266, 0)

    tst_gpio_pin_267 = 267
    #267 对应 T506S 扩展的 D0
    gpio.setup(tst_gpio_pin_267, gpio.OUT)
    gpio.set(tst_gpio_pin_267, 0)

    sys.exit(main(sys.argv, gpio))
